...
bool find_isosurface(vec3 ro, vec3 rd, float t_start, float t_end, out vec3 hit_point) {
    const int steps = 256;
    float prev_t = t_start;
    float prev_val = density(ro + rd * prev_t) - u_iso_value;

    for (int i = 1; i <= steps; ++i) {
        float t = mix(t_start, t_end, float(i) / float(steps));
        float val = density(ro + rd * t) - u_iso_value;
        bool crossed = (prev_val <= 0.0 && val >= 0.0) || (prev_val >= 0.0 && val <= 0.0);
        ...
        if (crossed) {
            ...
            for (int j = 0; j < 5; ++j) {
                float mid = 0.5 * (a + b);
                float mid_val = density(ro + rd * mid) - u_iso_value;
                float a_val = density(ro + rd * a) - u_iso_value;
                if ((a_val <= 0.0 && mid_val >= 0.0) || (a_val >= 0.0 && mid_val <= 0.0)) {
                    b = mid;
                } else {
                    a = mid;
                }
            }
            hit_point = ro + rd * (0.5 * (a + b));
            return true;
        }
...
void main() {
    ...
    // DEBUG: show bounding sphere as solid blue
    // Remove after confirming rays hit the sphere
    vec3 dbg_ro = ray_origin;
    vec3 dbg_rd = normalize(ray_dir);
    float dbg_r = max(5.0 * float(u_n * u_n) / u_Zeff, 3.0);
    vec3 dbg_oc = dbg_ro;
    float dbg_b = dot(dbg_oc, dbg_rd);
    float dbg_c = dot(dbg_oc, dbg_oc) - dbg_r * dbg_r;
    float dbg_disc = dbg_b * dbg_b - dbg_c;
    if (dbg_disc >= 0.0 && (-dbg_b + sqrt(dbg_disc)) > 0.0) {
      frag_color = vec4(0.0, 0.0, 1.0, 1.0);  // blue = ray hits sphere
      return;
    }
    frag_color = vec4(1.0, 0.0, 0.0, 1.0);  // red = ray misses sphere
    return;
    ...
    if (u_render_mode == 0) {
        ...
        float inv_steps = 1.0 / float(NUM_STEPS);
        float span = t1 - t0;
        float step_opacity = 3.0 / float(NUM_STEPS);
        float max_density = max(u_max_density, 1e-7);

        vec3 accum_color = vec3(0.0);
        float accum_alpha = 0.0;

        for (int i = 0; i < NUM_STEPS; ++i) {
            float t = t0 + span * (float(i) + 0.5) * inv_steps;
            vec3 p = ro + rd * t;
            float d = density(p);
            float normalized_d = clamp(d / max_density, 0.0, 1.0);
            float mapped = pow(normalized_d, u_gamma);
            float sample_alpha = mapped * step_opacity;
            vec3 sample_color = density_ramp(mapped);
        ...
        vec3 color = mix(kBackground, accum_color, clamp(accum_alpha, 0.0, 1.0));
